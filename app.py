...
# Updated clean_data function to include new rules
def clean_data(df, missing_threshold=0.2):
    df = df.copy()
    # Drop columns with date, text, or full text in the name
    ...
    # Drop columns that are text-like or have non-numeric dtypes
    for col in df.columns:
        if pd.api.types.is_string_dtype(df[col]) or pd.api.types.is_datetime64_any_dtype(df[col]):
            if col not in ['Company Name', 'SCM Practices', 'Technology Utilized', 'Supply Chain Agility', 'Supply Chain Integration Level']:
...
if uploaded_file:
    try:
        ...
        # Preprocessing and synthesis section
        ...
        # New: Categorize the data first before cleaning
        ...
        if st.sidebar.button("Generate Synthetic Data"):
            try:
                ...
                plot_correlation_heatmap(original_df, combined_synthetic_data, "Original", "Synthetic")

                st.write("### 3. Distribution Plots (KDE & Count)")
                # Get common columns for plotting
                common_cols = list(set(original_df.columns) & set(combined_synthetic_data.columns))
                plot_distributions(original_df, combined_synthetic_data, "Original", "Synthetic", common_cols)
